...
if (spotCount >= spots.length) return false;
...
if (spots[i] != null && spots[i].getSpotId().equals(spotId)) return spots[i];
}
return null;
}
...
for (int i = 0; i < spotCount; i++) {
if (spots[i] != null && spots[i].getSpotType().equals(spotType) && spots[i].isFree()) return spots[i];
}
return null;
}
